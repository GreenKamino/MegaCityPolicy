...
const OUT_DIR = path.resolve("presskit/screenshots");
...
async function ensureGameStarted(page) {
  // Try to bypass profile/new-game gating by setting localStorage first
  const has = await page.evaluate(() => {
    const keys = Object.keys(localStorage);
    return { keys, hasGame: keys.some((k) => k.startsWith("@megacity_slot")) };
  });
  return has;
}
...
(async () => {
  ...
  // Wait for the React app to actually render content. The Expo app renders into #root.
  ...
  console.log("App rendered. Initial state:", await ensureGameStarted(page));
  ...
  for (const shot of filteredShots) {
    ...
    // Wait for the expected text on this screen
    try {
      await page.waitForFunction(
        (re) => {
          const root = document.getElementById("root");
      ...
      console.log("  matched:", shot.waitForText);
    } catch {
