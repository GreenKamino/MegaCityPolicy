...
WORKFLOW_ASSERT_SCRIPT="$SCRIPT_DIR/assert-utility-parity-workflow-separation.sh"
TEMP_DIR="$(mktemp -d)"
...
if [[ "$url" == repos/*/actions/runs/*/jobs\?* ]]; then
  ...
  run_id="${run_id%%/*}"
  [[ "${FRESHNESS_JOB_NAME:-}" == "Packaged profile-slot cleanup (Windows x64)" ]] ||
    { printf 'unexpected utility parity job name: %s\n' "${FRESHNESS_JOB_NAME:-}" >&2; exit 2; }
...
run_case() {
  ...
  output="$(
    PATH="$MOCK_BIN:$PATH" \
      ...
      WORKFLOW_FILE=steam-profile-tmp-cleanup.yml \
      CURRENT_RUN_ID=999 \
      FRESHNESS_LABEL="Windows utility parity" \
      FRESHNESS_JOB_NAME="Packaged profile-slot cleanup (Windows x64)" \
...
run_case missing 1 FRESHNESS_MISSED

echo "Windows utility parity freshness fixture checks passed."